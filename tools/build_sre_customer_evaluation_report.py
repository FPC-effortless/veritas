...
def render_report(evaluation: dict[str, Any], customer_name: str, qualification: dict[str, Any] | None = None) -> str:
    lines = [
        ...
        "## Result",
        "",
        f"- Balanced accuracy: **{_pct(float(evaluation['balanced_accuracy']))}**",
        f"- Macro F1: **{_pct(float(evaluation['macro_f1']))}**",
        f"- Raw accuracy: **{_pct(float(evaluation['accuracy']))}**",
        f"- Majority-class baseline: **{_pct(float(evaluation['majority_baseline_accuracy']))}**",
        f"- Raw-accuracy lift over majority baseline: **{_pct(float(evaluation['accuracy_lift_over_majority']))}**",
        f"- Raw-accuracy 95% Wilson interval: **{_pct(float(evaluation['ci95_low']))} – {_pct(float(evaluation['ci95_high']))}**",
        ...
        "",
        "Balanced accuracy and macro F1 are the primary capability metrics because they prevent a dominant causal class from creating an artificially strong headline score. Raw accuracy remains a secondary descriptive metric.",
        "",
        ...
        "",
        "| Causal class | Cases | Precision | Recall | F1 | Recall 95% interval |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    for label, metrics in evaluation.get("per_class", {}).items():
        if metrics["n"]:
            ...
            interval = "n/a"
        lines.append(
            f"| {label} | {metrics['n']} | {_pct(metrics.get('precision'))} | "
            f"{_pct(metrics.get('recall', metrics.get('accuracy')))} | {_pct(metrics.get('f1'))} | {interval} |"
        )
