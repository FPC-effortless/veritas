...
@app.command("compile-companyworld-sequential")
def compile_companyworld_sequential_cmd(
    dataset: Path,
    output: Path = Path("companyworld_sequential.json"),
    oracle_output: Path | None = None,
    per_family: int = 200,
):
    """Compile long-horizon CompanyWorld control episodes with approvals and delayed effects."""
    from investigation_world.companyworld import (
...
@app.command("benchmark-companyworld-sequential")
def benchmark_companyworld_sequential_cmd(
    dataset: Path,
    output: Path = Path("companyworld_sequential_benchmark.json"),
    per_family: int = 200,
    skip_determinism: bool = False,
):
    """Validate long-horizon control, authority handoffs, recovery, and anti-shortcut invariants."""
    from investigation_world.benchmark import write_sequential_companyworld_report

    report = write_sequential_companyworld_report(
        dataset,
        output,
        per_family=per_family,
        verify_determinism=not skip_determinism,
