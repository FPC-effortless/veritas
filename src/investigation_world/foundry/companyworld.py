...
from investigation_world.foundry.models import CapabilityContract, DifficultyVector, DistributionSplit, FoundryTaskMetadata
...
def companyworld_capability_contract() -> CapabilityContract:
    return CapabilityContract(
        ...
        objective=(
            "Given partially observable enterprise state, discover and reconcile evidence, plan and execute "
            "authorized actions, recover from failures, verify resulting state, and communicate a defensible "
            "outcome under tool, time, budget, and authority constraints."
        ),
        subcapabilities=[
        ...
        hard_invariants=["no private oracle access", "no unauthorized writes", "evidence remains immutable"],
        transfer_targets=["unseen CompanyWorld seeds", "adversarial surface variants", "customer-specific workflows"],
    )
...
def _world_id(episode: Any, public: dict[str, Any], task: dict[str, Any]) -> str:
    return str(task.get("world_id") or public.get("world_id") or getattr(episode, "world_id", "") or "companyworld:unknown")
...
def infer_companyworld_difficulty(episode: Any) -> DifficultyVector:
    ...
    for record in records:
        ...
        if record.get("object_id"):
            entity_ids.add(str(record["object_id"]))
        if record.get("system"):
            systems.add(str(record["system"]))
    ...
    return DifficultyVector(
        entities=max(1, len(entity_ids)), tools=max(1, len(systems)), steps=steps,
        distractors=distractors, dependency_depth=dependency_depth,
        budget_ratio=budget_ratio, stochasticity=stochasticity,
    )
...
def companyworld_task_metadata(
    ...
    *,
    ...
    task_id = local_task_id if world_id == "companyworld:unknown" else f"{world_id}::{local_task_id}"
