...
def _classify_row(
    ...
    *,
    ...
    case_hash = stable_hash({"row": row_number, "public": public_payload})[:12]
    case_id = f"{profile.source_id}-{row_number:08d}-{case_hash}"
...
def audit_structured_investigation_corpus(
    ...
    passed = not overlap and duplicate_ids == 0 and not leaked_cases
    return {
        "passed": passed,
