...
def _remap_ids(
    ...
    *,
    ...
    world_id = f"{domain_token}-{split.value}-{seed:010d}-{index:05d}"
    task_id = f"{world_id}-task"
    record_map = {
        record.record_id: f"{domain_token[:12]}-{split.value[:4]}-{seed:010d}-r{position:03d}"
...
def _difficulty(
    ...
    *,
    ...
    entity_ids = {
        ...
        related
        for record in episode.records
        for related in record.related_object_ids
    }
    ...
    return DifficultyVector(
        ...
        budget_ratio=round(episode.oracle.max_cost / max(1, sum(action.cost for action in episode.task.available_actions)), 3),
        stochasticity=0.0,
...
def distribution_manifest(
    ...
    *,
    ...
    public_payloads = [_public_case_payload(case) for case in cases]
    private_payloads = [_private_case_payload(case) for case in cases]
    return OperationalDistributionManifest(
        distribution_id=f"veritas-operational-production-{config.seed}",
...
def validate_operational_distribution(
    ...
    *,
    ...
    for case in cases:
        public_text = json.dumps(case.episode.public_payload(), sort_keys=True, default=str)
...
def public_distribution_payload(
    ...
    *,
    ...
    return {
        ...
        "manifest": manifest.model_dump(mode="json"),
        "episodes": [_public_case_payload(case) for case in cases],
    }
