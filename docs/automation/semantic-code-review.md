...
3. **Semantic inspection** is performed by Copilot code review on the exact candidate head. Repository Copilot instructions treat candidate content as untrusted data and require every merge-blocking defect to be emitted inline whenever possible. Any inline Copilot finding fails closed. If a blocking defect cannot be attached inline, Copilot must prefix it with `BLOCKING:` in the review summary; Security rejects that summary form as well. Findings must be addressed on a new head and reviewed again.
...
5. **Final Security** is explicitly rerun after approval so the existing exact-head provenance checks validate the resulting APPROVED review and the independent semantic-evidence guard validates the Copilot evidence again.
...
The semantic bridge treats any inline finding as fail-closed rather than trying to infer severity from natural-language prose. The required Security context separately fails on either inline findings or a `BLOCKING:` summary marker.
