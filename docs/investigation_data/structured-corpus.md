...
`investigation_world.investigation_data.SourceCatalog` remains authoritative for source identity, acquisition policy, AI-use policy, redaction requirements, truth semantics, and declared artifacts.

A `StructuredSourceProfile` must bind to an existing canonical `source_id` and `source_artifact_id`. Compilation fails closed when either identity is absent from the catalog, the source is blocked, metadata-only policy is incompatible with the declared artifact, or a required rights/redaction review reference is missing.
