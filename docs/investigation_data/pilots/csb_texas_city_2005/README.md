...
| 2007-03-20 | Final investigation report | document/PDF | https://www.csb.gov/assets/1/20/csbfinalreportbp.pdf |
...
3. `2007-03-21T00:00:00Z`: preliminary + organizational + final report;
4. `2008-03-22T00:00:00Z`: all documents + the official CSB video.
...
`review_record.json` scopes `review-csb-texas-city-link-only-v1` to public, link-only use and excludes named individuals from the normalized task state. Any later local acquisition, transcript extraction, frame extraction, redistribution, or training use requires a separate artifact-level review.
...
- the final report appears before its documented 2007 release;
...
- the final CSB conclusion is serialized into the public episode before its evidence is available;
