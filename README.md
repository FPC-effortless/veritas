...
- FastAPI episodes are isolated. Each episode has its own hidden world/oracle, search index, task, and investigation budget; there is no agent-accessible filesystem world loader.
...
A privileged orchestrator creates an episode with `POST /admin/episodes`, supplying a hidden `CanonicalWorld` and optionally a matching public task/private oracle. If omitted, the server generates a task/oracle pair internally. The response contains only the episode ID and public task.
...
This remains a synthetic benchmark substrate, not a complete commercial environment. Renderer realism, larger adversarial transformation libraries, stronger evidence entailment models, benchmark manifest tooling, private-eval storage, and external harness adapters remain follow-on work.
